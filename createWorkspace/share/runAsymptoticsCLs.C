...
//  double mu_up_p2_approx = sigma*(ROOT::Math::gaussian_quantile(1 - target_CLs, 1) + 2);
 // double mu_up_p1_approx = sigma*(ROOT::Math::gaussian_quantile(1 - target_CLs, 1) + 1);
 // double mu_up_n1_approx = sigma*(ROOT::Math::gaussian_quantile(1 - target_CLs, 1) - 1);
 // double mu_up_n2_approx = sigma*(ROOT::Math::gaussian_quantile(1 - target_CLs, 1) - 2);
...
double calcCLs(double qmu_tilde, double sigma, double mu)
{
  ...
  if (pb == 1) return 0.5;
  return pmu/(1-pb);//Loan commented out for checking with Will March 3rd, 2016
 //return pmu;
}
